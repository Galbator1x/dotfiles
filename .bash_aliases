alias p='sudo sh pycharm.sh'
alias i='sudo sh idea.sh'

alias sorti='isort -rc .'
alias y='yandex-disk'

alias va='source ../activate'
alias vbp='source ~/projects/brand_protection/bp/bin/activate'
alias vshop='source ~/projects/venv/shop/bin/activate'
alias vp36='source ~/projects/venv/p36/bin/activate'
alias vb='source ~/projects/venv/blago/bin/activate'
alias pm='python manage.py'
alias pmr='python manage.py runserver'
alias pmm='python manage.py migrate'
alias pmmk='python manage.py makemigrations'
alias pmu='python manage.py upgrade'

alias cd1='cd ..'
alias cd2='cd ../../'
alias cd3='cd ../../../'
alias cdp='cd ~/projects/'
alias cdb='cd ~/projects/blago/'
alias cdbp='cd ~/projects/brand_protection/bp_web/bp_web/'
alias cdd='cd ~/projects/devman/'
alias cddot='cd ~/projects/dotfiles/'
alias cdmkv='cd ~/Videos/mkv/'

alias pstart='sudo /etc/init.d/postgresql start'
alias pstop='sudo /etc/init.d/postgresql stop'
alias prestart='sudo /etc/init.d/postgresql restart'
alias pstatus='sudo /etc/init.d/postgresql status'
alias pshell='sudo -u postgres psql'

alias cl='clear'
alias gs='git status'
alias ga='git add'
alias gb='git branch'
alias gc='git commit -m'
alias gp='git push'
alias gpu='git push --set-upstream origin'
alias gph='git push heroku master'
alias gd='git diff'
alias go='git checkout'
alias gk='gitk --all&'
alias gx='gitx --all'
alias gh='git log --pretty=format:"%h %ad | %s%d [%an]" --graph --date=short'

